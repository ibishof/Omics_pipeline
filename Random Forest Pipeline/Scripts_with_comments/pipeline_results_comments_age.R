...
msdss_plot <- pipeline_results("age","age_lp",iters=0:119)
#Plots out of bag error vs interations
msdss_plot

# Specifing best_iter places line at that location
msdss_plot <- pipeline_results("age","age_lp",iters=0:119,
                       best_iter=100) #Orginal 109
msdss_plot

# Pick the iteration that will be used for the final model developed
msdss_iter <- 100 #Orginal 103

# Loads in the set of ratios/somamers that were used at the specified iteration,
# then append them to the metadata
age_var <- readLines(paste('C:\\Users\\bishofij\\Desktop\\here\\age\\iter',msdss_iter,"_ratios.txt",sep=""))
soma_untreated <- soma_untreated %>% ratio_append(age_var)
...
nvars <- floor(3*sqrt(length(age_var)))
age_mod <- ranger(data=as.data.frame(soma_train[,c("age_lp",age_var)]), 
                    num.trees=40000,mtry=nvars,importance='impurity',
              seed=223,dependent.variable.name="age_lp",write.forest = TRUE)

# Save the model object for later use
saveRDS(age_mod,"age_mod.rds")

# Can be loaded with the following:
msdss_mod <- readRDS("msdss_mod.rds")

######################################################################
##################   Add predictions to Dataset ######################
######################################################################

#Makes prediction using msdss_mod
# For the training data, using the out-of-bag predictions from the model object
#msdss is observed value
#soma_msdss is predicted value
soma_train$soma_age <- age_mod$predictions

# For the validation data, create the predictions using the predict function as so
#List of predictions check this object out
#This will be plotted later p[redicted sseverity vs measure severity]
soma_test$soma_age <- predict(age_mod,soma_test)$predictions

# Merge the two back together
#has both training and test set. With labels
soma_untreated <- bind_rows(soma_train,soma_test)


write.csv(soma_train, "soma_train_age.csv")
write.csv(soma_test, "soma_test_age.csv")
write.csv(soma_untreated, "soma_untreatedb_age.csv")
...
age_imp_dat <- tibble(ratio = names(age_mod$variable.importance),
                        importance = as.numeric(age_mod$variable.importance))
write.csv(age_imp_dat, "age_imp_dat.csv")
...
with(soma_test,my_scatterplot(soma_age,age_lp,main="age_lp"))

corr <- cor.test(x=soma_test$age_lp, y=soma_test$soma_age, method = 'spearman')
corr= as.numeric(corr$estimate)
cat("age_lp","Spearman's Correlation = ",corr)
