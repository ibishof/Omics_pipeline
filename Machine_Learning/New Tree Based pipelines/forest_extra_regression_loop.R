...
# If rerunning pipeline start here
...
# Only first iteration starts here
# Bring in training data
#s et working directory
# Auto split data into Training and Testing
if (isFALSE(exists("random_forest_input"))){
  setwd("C:\\Users\\kqjf682\\Omics_pipelines\\CKD\\A003_1000\\corrected")
  data <- read.csv("cleaned_eGFR.csv", check.names=FALSE)
  data <- data[,-1]
  data_size=nrow(data)
...
rating_mod <- ranger(data=training, # The full dataset containing predictors and response variables
              ...
              importance='permutation',  # Type of variable importance measure to calculate
...
plot(predictions$predictions, validation$Diagnosis, main= Title,
     xlab="Predictions", ylab="True eGFR", cex.lab = 1.5, pch=19)
...
write.csv(list_train_cor, "list_train_cor_permutation_extra2.csv")
write.csv(list_test_cor, "list_test_cor_permuatation_extra2.csv")
