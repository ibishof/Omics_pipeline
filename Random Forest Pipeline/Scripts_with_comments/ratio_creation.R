...
soma_untreated <- soma_untreated %>% 
  select(sampleid,which(names(soma_untreated) %in% markers))
...
soma_dt[,sampleid := soma_untreated$sampleid]
setcolorder(soma_dt,c("sampleid"))

# Add individual somamers to data.table, merge with ratios, and save
markers_dt <- soma_untreated %>% 
  as.data.table()
soma_dt <- merge(soma_dt,markers_dt,by="sampleid")
saveRDS(soma_dt,"./data/ms_ratios_20190910.rds")
